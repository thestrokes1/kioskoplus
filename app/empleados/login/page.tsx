...
export default function EmpleadosLoginPage() {
  ...
  async function onSubmit(data: LoginInput) {
    ...
    if (result.role === 'empleado' || result.role === 'admin') {
      router.refresh()
      router.push('/empleados/ventas')
    } else {
      setError('No tenés acceso al panel de empleados')
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-8 shadow-sm">
        <div className="mb-6 text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Panel de Empleado</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">KioskoPlus — acceso restringido</p>
        </div>
